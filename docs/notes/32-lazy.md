...
```Java title="Logger v0.1 (eager evaluation)"
class Logger {
...
```Java title="Logger v0.2 (with Producer)"
class Logger {
  enum LogLevel { INFO, WARNING, ERROR };
...
While other languages such as Scala has native support for lazy variables, Java does not.  So let's build a simple one here.  (You will build a more sophisticated one in Programming Exercise 6) 
...
```Java title="Logger v0.3 (with Lazy)"
class Logger {
  enum LogLevel { INFO, WARNING, ERROR };
...
If `loginMessage` is used in multiple places, then memoization ensures that the invocation of `System.getProperty("user.name")` and the concatenation of the strings is done only once.
