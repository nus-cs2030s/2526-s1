...
[^1]: The `InfiniteList` from programming exercise does not currently have `flatMap`.  But we can easily add that.
...
The three laws below are all related to the context.  In particular, we need to ensure that the composition`context` is associative.  We also want the `of` method to "do nothing" to the context.  This is captured by the idea of "identity" context.  Finally, we want `flatMap` to be associative with respect to the context.
